...
class TestTicketViewer(unittest.TestCase):
    ...
    def test_get_selected_ticket(self):
        ...
        return: N/A
        """    
        if self.response_data["count"] > 0:
            ticket_id = self.response_data["tickets"][0]["id"]
            with app.test_client() as client: 
                selected_ticket_response = client.get("/getSelectedTicket?user=" + username + "&pwd=" + password + "&ticket_id=" + str(ticket_id))
    ...
    def test_get_user_name(self):
        ...
        if self.response_data["count"] > 0:
            ...
            if requester_id != '':
                with app.test_client() as client: 
                    user_info_response = client.get("/getUserName?user=" + username + "&pwd=" + password + "&user_id=" + str(requester_id))
